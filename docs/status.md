...
> **TL;DR.** Crime, walkability, and street lights are cleaned. Scoring engine is functional and routing produces three distinct routes (fastest, safest, balanced). Next: Ruhan builds Streamlit frontend. Bike lanes listed as a potential future data source, not in scope for this iteration.
...
| Ajay | Balanced route: safety score cutoff + fastest from there (May 12); test 5 start/end pairs and screenshot on Discord (May 5) | [`design_document.md`](design_document.md) §7 | in progress |

## Big picture

Crime, walkability, and street lights are cleaned. Scoring engine is functional and routing is validated. Bike lanes are listed as a potential future data source (not in scope for this iteration). The next milestone is the Streamlit frontend.
...
| UCSD Clery aggregates extracted | Source: UCSD Annual Security & Fire Safety Report 2025 §XVI (22 offenses x 3 years). Intended as **validator** for the daily-log scrape, not as a point feature. File not yet committed to repo. |
...
| UCSD `campus_incident_score` (F10) | daily UCSD Police log scrape not yet downloaded + location-string lookup table not yet built. If campus crime data not available, mention during presentation as next steps or ask DS3 to help (May 12 meeting, Max with Vanshika). |
...
| 2026-04-28 (Tue, 9:00-10:00pm) | Assigned work: Matthew (weighted scores), Ruhan (test scoring on sample routes), Max (cleaning remaining datasets), Ajay (comparing weighted scores). Team norms: text after every update on the Discord, react to every message |
